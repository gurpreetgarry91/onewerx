...
(function ($) {
    ...
    wp.customize('fw_options[stunning_bg_image]', function (value) {
        value.bind(function (newval) {
            newval = JSON.parse(newval);
            if (newval[0].value == 'custom') {
                $stunning.css({'backgroundImage': 'url(' + newval[3].value + ')'});
            } else {
                var templateUrl = theme_vars.templateUrl;
                $stunning.css({'backgroundImage': 'url(' + templateUrl + '/images/' + newval[1].value + '.png)'});
            }
        });
    });
    wp.customize('fw_options[stunning_bg_cover]', function (value) {
        value.bind(function (newval) {
            newval = JSON.parse(newval);
            if (newval[0].value == 'true') {
                $stunning.css({'backgroundSize': 'cover'});
            } else {
                $stunning.css({'backgroundSize': 'inherit'});
            }
        });
    });
